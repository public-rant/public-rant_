...
```shell
  docker buildx build -t rmse/conductor:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install rmse/conductor:latest
```
...
```shell
  docker extension dev ui-source rmse/conductor:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug rmse/conductor:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset rmse/conductor:latest
```
...
```shell
docker extension update rmse/conductor:latest
```
...
```shell
docker extension rm rmse/conductor:latest
```
